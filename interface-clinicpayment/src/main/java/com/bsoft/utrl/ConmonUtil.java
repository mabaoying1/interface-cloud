...
import com.bsoft.gwClient.BSXmlWsEntryClassService;
import com.bsoft.gwClient.BSXmlWsEntryClassServicePortType;
import com.bsoft.gwClient.InvokeRequest;
import com.bsoft.gwClient.InvokeResponse;
import com.bsoft.mapper.identity.IdentityMapper;
import com.bsoft.util.HttpClientUtil;
...
public class ConmonUtil {
    ...
    private static Logger logger = LoggerFactory.getLogger(ConmonUtil.class);
     ...
     * @Param null
     */
    public String getSuccessZd(Object data) {
        JSONObject json = new JSONObject();
        json.put("status", "0");
        json.put("code", "1");
        json.put("info", data);
     ...
     * @author
     */
    public Map<String,Object> getDiscount(Map<String, Object> req) {
        JSONObject json = new JSONObject();
        json.putAll(req);
        String resultString="";
        logger.info("打折入参："+json.toString());
        try {
            resultString = HttpClientUtil.doPostJson(httpUrl + "GetChargeFeeReduceResult", json.toString());
        }catch(Exception e){
            throw new CommonException("查询打折信息错误");
        }
        //logger.info("打折返回："+resultString);
        Map<String, Object>  result = JSONUtil.toHashMap(resultString);
        if(!result.get("CODE").toString().equals("0")){
            throw new CommonException("查询打折信息错误："+result.get("MSG"));
        }
        return result;
    }
    /**
     ...
     * @author
     */
    public void pusPlatform(String service, String SourceId, String EtlTaskCode, String SourceTable,String YJXH) {
        try {
            logger.info("开始调用平台" );
            String message = "";
            //拼接平台需要的xml
            ...
            MsgHeader.put("MsgType", "ETL_0101");
            MsgHeader.put("MsgVersion", "3.0");
            MsgHeader.put("InteractionId", service);
            MsgHeader.put("Sync", "0");
            LogInfo.put("EtlTaskCode", EtlTaskCode);
            LogInfo.put("SourceId", SourceId);
            LogInfo.put("LogType", "1");
            //获取当前时间
            ...
            LogInfo.put("EffectiveTime", EffectiveTime);
            LogInfo.put("SourceTable", SourceTable);
            LogInfo.put("YJXH", YJXH);
            BSXml.put("MsgHeader", MsgHeader);
            BSXml.put("LogInfo", LogInfo);
            message = XmlMapUtils.formatXml(XmlMapUtils.map2xml(BSXml, "BSXml"));
            logger.info("传入平台:" + message);
//            // 创建动态客户端
//            JaxWsDynamicClientFactory dcf = JaxWsDynamicClientFactory.newInstance();
//            Client client = dcf.createClient(wsdlUrl);
//            Object[] objects = new Object[0];
//            Endpoint endpoint = client.getEndpoint();
//            QName opName = new QName(endpoint.getService().getName().getNamespaceURI(), "invoke");
//            logger.info("传入平台:" + message);
//            objects = client.invoke(opName, service, "test", "test", message);
//            logger.info("平台返回:" + objects[0]);
            //创建一个MobileCodeWS工厂
            BSXmlWsEntryClassService factory = new BSXmlWsEntryClassService();
            //根据工厂创建一个MobileCodeWSSoap对象
            BSXmlWsEntryClassServicePortType BSXmlWsEntryClass = factory.getBSXmlWsEntryClassServicePort();
            BindingProvider bindingProvider = (BindingProvider) BSXmlWsEntryClass;
            Map<String, Object> requestContext = bindingProvider.getRequestContext();
            requestContext.put("com.sun.xml.internal.ws.connection.timeout", 5 * 1000);//建立连接的超时时间为5秒
            requestContext.put("com.sun.xml.internal.ws.request.timeout", 5 * 1000);//指定请求的响应超时时间为5秒
            ...
            InvokeRequest parameters=new  InvokeRequest();
            parameters.setParameter(message);
            parameters.setService(service);
            parameters.getPwd();
            parameters.getUrid();
            InvokeResponse invokeResponse=BSXmlWsEntryClass.invoke(parameters);
            //invoke(service, "", "", message);
            logger.info("平台返回:" + invokeResponse.getOutput());
        } catch (Exception e) {
