...
@Configuration
@MapperScan(basePackages = "com.bsoft.mapper.pacs", sqlSessionFactoryRef = "db4SqlSessionFactory")
public class DataSourceConfigPacs {
    // 将这个对象放入Spring容器中
    ...
    public SqlSessionFactory db4SqlSessionFactory(@Qualifier("db4DataSource") DataSource datasource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        bean.setDataSource(datasource);
        bean.setMapperLocations(
                // 设置mybatis的xml所在位置
                new PathMatchingResourcePatternResolver().getResources("classpath*:mapping/pacs/*.xml"));
    ...
    public SqlSessionTemplate db4sqlsessiontemplate(
            @Qualifier("db4SqlSessionFactory") SqlSessionFactory sessionfactory) {
